...
from PySide6.QtWidgets import (
    QMainWindow, QToolBar, QApplication, QDialog, QWidget, QHBoxLayout, QFormLayout,
    QToolButton, QLineEdit, QFileDialog, QCheckBox, QLabel, QMessageBox, QPushButton
)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def initUI(self):
        ...
        self.lip_sync_btn = QToolButton()
        self.lip_sync_btn.setText("视频嘴唇对齐")
        self.lip_sync_btn.setToolTip("利用音频对齐视频唇形")
        self.lip_sync_btn.setFixedWidth(101)
        self.lip_sync_btn.setStyleSheet("""
            QToolButton {
        ...
        """)
        self.toolBar.addWidget(self.lip_sync_btn)
        self.lip_sync_btn.clicked.connect(self.show_lip_sync_settings)
    ...
    def show_lip_sync_settings(self):
        dialog = LipSyncSettingsDialog(self)
        if dialog.exec_() == QDialog.Accepted:
            settings = dialog.get_settings()
            # 保存设置或打印
            print("视频嘴唇对齐:", settings)
...
class LipSyncSettingsDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("视频嘴唇对齐")
        ...
        layout.addRow("视频路径:", self._with_button(self.video_path_input, video_browse_btn))

        # 音频路径选择
        self.audio_path_input = QLineEdit(self)
        audio_browse_btn = QPushButton("浏览")
        audio_browse_btn.clicked.connect(self.browse_audio)
        layout.addRow("音频路径:", self._with_button(self.audio_path_input, audio_browse_btn))

        # 是否开启超分辨率
        self.sr_checkbox = QCheckBox("开启超分辨率(Real-ESRGAN)")
        self.sr_checkbox.setChecked(True)
        layout.addRow(self.sr_checkbox)
        ...
        # 开始按钮
        self.start_btn = QPushButton("开始嘴唇对齐")
        self.start_btn.clicked.connect(self.run_lip_sync_process)
    ...
    def _with_button(self, widget, button):
        container = QWidget()
        hl = QHBoxLayout(container)  # 这里要指定父widget
        hl.addWidget(widget)
        hl.addWidget(button)
        return container
    ...
    def browse_audio(self):
        path, _ = QFileDialog.getOpenFileName(self, "选择音频文件", "", "音频文件 (*.wav *.mp3)")
        if path:
            self.audio_path_input.setText(path)

    def run_lip_sync_process(self):
        video_path = self.video_path_input.text().strip()
        audio_path = self.audio_path_input.text().strip()
        use_sr = self.sr_checkbox.isChecked()

        if not video_path or not audio_path:
            QMessageBox.warning(self, "错误", "请先选择视频和音频文件路径！")
            return

        # 当前脚本文件所在目录
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # Wav2Lip的绝对路径（根据相对位置向上两级，再进入Wav2Lip）
        base_dir = os.path.abspath(os.path.join(current_dir, "..", "..", "Wav2Lip"))

        out_video = os.path.join(base_dir, "output_videos_wav2lip", "output.mp4")
        frames_path = os.path.join(base_dir, "frames_wav2lip", "output")
        frames_hd_path = os.path.join(base_dir, "frames_hd", "output")
        final_output = os.path.join(base_dir, "data", "output.mkv")
        pred_path = os.path.join(base_dir, "data", "lq")
        gt_path = os.path.join(base_dir, "data", "gt")

        # 确保路径存在
        os.makedirs(os.path.dirname(out_video), exist_ok=True)
        os.makedirs(frames_path, exist_ok=True)
        os.makedirs(frames_hd_path, exist_ok=True)
        os.makedirs(os.path.dirname(final_output), exist_ok=True)

        # 1. 运行Wav2Lip推理
        cmd_wav2lip = [
            sys.executable, os.path.join(base_dir, "inference.py"),
            "--checkpoint_path", os.path.join(base_dir, "checkpoints", "wav2lip_gan.pth"),
            "--segmentation_path", os.path.join(base_dir, "checkpoints", "face_segmentation.pth"),
            "--sr_path", os.path.join(base_dir, "checkpoints", "esrgan_yunying.pth"),
            "--face", video_path,
            "--audio", audio_path,
            "--save_frames",
            "--gt_path", gt_path,
        ...
        try:
            self.status_label.setText("开始嘴唇对齐推理...")
            QApplication.processEvents()
            subprocess.run(cmd_wav2lip, check=True)

            if use_sr:
                # 2. 获取帧率
                self.status_label.setText("提取视频帧率...")
                QApplication.processEvents()
                fps_cmd = f'ffprobe -v 0 -of csv=p=0 -select_streams v:0 -show_entries stream=r_frame_rate "{out_video}"'
                fps_output = subprocess.check_output(fps_cmd, shell=True, text=True)
                from fractions import Fraction
                fps = float(Fraction(fps_output.strip()))

                # 3. 拆帧
                self.status_label.setText("拆分视频帧...")
                QApplication.processEvents()
                subprocess.run([
                    sys.executable, os.path.join(base_dir, "video2frames.py"),
                    "--input_video", out_video,
                    "--frames_path", frames_path
                ], check=True)

                # 4. 超分处理
                self.status_label.setText("开始超分辨率处理...")
                QApplication.processEvents()
                subprocess.run([
                    sys.executable, os.path.join(base_dir, "Real-ESRGAN-master", "inference_realesrgan.py"),
                    "-n", "RealESRGAN_x4plus",
                    "-i", frames_path,
                    "--output", frames_hd_path,
                    "--outscale", "3.5",
                    "--face_enhance"
                ], check=True)

                # 5. 合成高清视频（用超分后的帧）
                self.status_label.setText("合成高清视频（超分后）...")
                QApplication.processEvents()
                ffmpeg_cmd = [
                    "ffmpeg",
                    "-r", str(fps),
                    "-i", os.path.join(frames_hd_path, "frame_%05d_out.jpg"),
                    "-i", audio_path,
                    "-vcodec", "libx264",
                    "-crf", "25",
                    "-preset", "medium",
            ...
            self.status_label.setText("处理完成！输出文件: " + final_output)
            QMessageBox.information(self, "完成", "嘴唇对齐视频生成完成！")

        except subprocess.CalledProcessError as e:
            self.status_label.setText("发生错误: " + str(e))
            QMessageBox.critical(self, "错误", "运行过程出现错误，请查看控制台或日志。")
