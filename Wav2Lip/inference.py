...
parser.add_argument('--sr_path', type=str, default='checkpoints/esrgan_yunying.pth',
                    help='Name of saved checkpoint of super-resolution network')

parser.add_argument('--face', type=str, default='input_videos/mona.mp4',
                    help='Filepath of video/image that contains faces to use')
parser.add_argument('--audio', type=str, default='input_audios/ai.wav',
...
def face_detect(images):
    ...
    for rect, image in zip(predictions, images):
        if rect is None:
            cv2.imwrite(os.path.join(BASE_DIR, 'temp/faulty_frame.jpg'),
...
def main():
    ...
    if args.save_as_video:
        gt_out = cv2.VideoWriter(os.path.join(BASE_DIR, 'temp/gt.avi'), cv2.VideoWriter_fourcc(*'DIVX'), fps,
                                 (384, 384))
        pred_out = cv2.VideoWriter(os.path.join(BASE_DIR, 'temp/pred.avi'), cv2.VideoWriter_fourcc(*'DIVX'), fps,
                                   (96, 96))
    ...
    command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(args.audio, os.path.join(BASE_DIR, 'temp/result.avi'),
                                                                  args.outfile)
    ...
    if args.save_frames and args.save_as_video:
        ...
        command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(args.audio, os.path.join(BASE_DIR, 'temp/gt.avi'),
                                                                      args.gt_path)
        subprocess.call(command, shell=platform.system() != 'Windows')

        command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(args.audio,
                                                                      os.path.join(BASE_DIR, 'temp/pred.avi'),
                                                                      args.pred_path)
        subprocess.call(command, shell=platform.system() != 'Windows')
